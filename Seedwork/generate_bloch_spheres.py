...
def plot_multi_vector(circuit, filename):
    simulator = Aer.get_backend('statevector_simulator')
    statevector = execute(circuit, backend = simulator, shots=1).result().get_statevector()
    rounded_state_vector = []
    for complex_number in statevector:
        new_complex = complex(round(complex_number.real, 2), round(complex_number.imag, 2))
...
for i in range(0, NUM_ITERATION):
    ...
    quantum_circuit.h(1)
    plot_multi_vector(quantum_circuit, 'Seedwork/plots/bloch_spheres_' + str(i))
